...
function setState(el, state) {
    ...
    var titleEl = el.querySelector(SELECTORS.title);
    var btn = titleEl.querySelector('button');
    btn.setAttribute('aria-expanded', state === 'open' ? 'true' : 'false');
    var label = el.getAttribute(state === 'open'
...
function openByHash(hash) {
    try {
        ...
        if (targetEl) {
            var matchEl = targetEl.closest(SELECTORS.el);
            if (matchEl) {
...
function initTargetedContentFor(el) {
    var titleEl = el.querySelector(SELECTORS.title);
    var contentEl = el.querySelector(SELECTORS.content);
    ...
    var createToggleButton = function () {
        titleEl.innerHTML = "<button type=\"button\" class=\"".concat(CLASS_NAMES.button, "\" aria-expanded=\"false\" aria-controls=\"").concat(contentEl.id, "\"\n      aria-label=\"").concat(el.getAttribute('data-descriptive-label-show'), "\">\n        ").concat(titleEl.innerHTML, "\n        <svg class=\"").concat(CLASS_NAMES.icon, "\" viewBox=\"0 0 10 10\" aria-hidden=\"true\" focusable=\"false\">\n          <rect class=\"").concat(CLASS_NAMES.iconVertLine, "\" height=\"8\" width=\"2\" y=\"1\" x=\"4\" />\n          <rect height=\"2\" width=\"8\" y=\"4\" x=\"1\" />\n        </svg>\n      </button>");
    };
    /**
    ...
    var createCloseButton = function () {
        var ariaLabel = el.getAttribute('data-descriptive-label-hide');
        var closeButton = "<hr class=\"cads-separator\" />\n        <button type=\"button\" aria-label=\"".concat(ariaLabel, "\" class=\"cads-linkbutton ").concat(CLASS_NAMES.closeButton, "\">\n          ").concat(el.getAttribute('data-close-label'), "\n        </button>");
        contentEl.insertAdjacentHTML('beforeend', closeButton);
    };
    el.classList.add(CLASS_NAMES.toggleable);
    createToggleButton();
    var toggleButtonEl = el.querySelector(".".concat(CLASS_NAMES.button));
    toggleButtonEl.addEventListener('click', function () {
        var currentlyExpanded = toggleButtonEl.getAttribute('aria-expanded') === 'true' || false;
        setState(el, currentlyExpanded ? 'closed' : 'open');
    });
    if (!el.classList.contains(CLASS_NAMES.noClose)) {
        createCloseButton();
    }
    var closeButtonEl = contentEl.querySelector(".".concat(CLASS_NAMES.closeButton));
    if (closeButtonEl) {
        closeButtonEl.addEventListener('click', function () {
            var matchEl = closeButtonEl.closest(SELECTORS.el);
            if (matchEl) {
...
function initTargetedContent() {
    var targetedContentEls = document.querySelectorAll(SELECTORS.el);
    if (targetedContentEls.length > 0) {
        for (var i = 0; i < targetedContentEls.length; i++) {
            var el = targetedContentEls[i];
            /**
...
exports.default = initTargetedContent;
